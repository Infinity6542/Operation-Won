...
class HomeScreenState extends State<HomeScreen> {
  ...
  WebSocketChannel? webSocketChannel; // Audio recording and playback
  ...
  // Handle PTT button press - start recording and sending audio
  Future<void> _onPushToTalkPressed() async {
    ...
    if (await _audioRecorder.hasPermission()) {
      ...
      await _audioRecorder.start(
        recorderCallback: (data) {
          if (isConnected && webSocketChannel != null) {
            // Send audio data to server as binary message
      ...
      _showSnackBar('PTT active - Recording and streaming audio');
    } else {
      _showSnackBar('Microphone permission denied');
    }
  }

  // Handle PTT button release - stop recording and send stop signal
  Future<void> _onPushToTalkReleased() async {
    if (isRecording) {
      await _audioRecorder.stop();

      // Send PTT stop signal to server
      if (webSocketChannel != null) {
  ...
  void _handleIncomingAudio(Uint8List audioData) {
    ...
    // Note: In a real app, you'd need to buffer and process this properly
    // This is a simplified version that may not work smoothly in all cases
    try {
      // Convert raw bytes to a playable audio source
      // This approach may need refinement for smooth playback
      _audioPlayer.setAudioSource(StreamAudioSource(
        StreamController<List<int>>()..add(audioData),
        contentType: 'audio/opus', // Match the format from the server
      ));
      _audioPlayer.play();

      // Reset the receiving flag after a short delay (this is a simplification)
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _audioPlayer.setAudioSource(StreamAudioSource(
          StreamController<List<int>>()..add(audioBytes),
          contentType: 'audio/opus', // Match the format from the server
        ));
...
// Custom audio source for just_audio to play from memory bytes
class StreamAudioSource extends StreamAudioSourceBase {
  final StreamController<List<int>> _controller;
  final String contentType;

  StreamAudioSource(this._controller, {required this.contentType});
