...
class _EnhancedPTTButtonState extends State<EnhancedPTTButton>
  ...
  void _handlePanUpdate(DragUpdateDetails details, CommsState commsState) {
    ...
    if (distance > _swipeThreshold) {
      if (delta.dy > _swipeThreshold && _currentGesture == PTTGestureType.hold) {
        // Swipe down - Leave channel
        _handleSwipeDown(commsState);
      } else if (delta.dy < -_swipeThreshold && _currentGesture == PTTGestureType.hold) {
  ...
  Widget build(BuildContext context) {
    return Consumer<CommsState>(
      builder: (context, commsState, child) {
        ...
        final activationZoneSize = widget.size * widget.activationZoneMultiplier;

        return SizedBox(
          width: activationZoneSize,
          height: activationZoneSize,
          child: GestureDetector(
            onPanStart: canUsePTT ? (details) => _handlePanStart(details, commsState) : null,
            onPanUpdate: canUsePTT ? (details) => _handlePanUpdate(details, commsState) : null,
            onPanEnd: canUsePTT ? (details) => _handlePanEnd(details, commsState) : null,
            child: Center(
  ...
  Color _buildShadowColor(CommsState commsState, bool canUsePTT) {
    if (!canUsePTT) return Colors.grey.withOpacity(0.3);
  ...
  Widget _buildIcon(CommsState commsState, bool canUsePTT) {
    ...
    } else if (_currentGesture == PTTGestureType.swipeUp || _showEmergencyCountdown) {
      iconData = LucideIcons.circleAlert;
      iconColor = Colors.white;
    } else if (_currentGesture == PTTGestureType.swipeDown || _showLeaveConfirmation) {
